...
public struct MigrationV1: GraphMigration {
    public func handlePhase(_ phase: GraphMigrationManager.GraphLifecyclePhase, configuration: GraphStoreConfiguration?, graph: Graph?, context: GraphMigrationContext?, completion: @escaping (GraphMigrationResult) -> Void) {
        switch phase {
        case .preInit:
            do {
                guard let config = configuration else {
                    completion(.error(NSError(domain: "MigrationV1", code: 1, userInfo: [
                        NSLocalizedDescriptionKey: "Missing GraphStoreConfiguration during MigrationV1 preInit phase"
                    ])))
                    return
                }
                _ = try Self.execute(configuration: config)
                completion(.done)
            } catch {
        ...
        case .postInit:
            ...
            if let graph = graph, let url = graph.runtimeStoreURL {
                
                GraphValueTransformer.register()
                Self.applyAppDataVersion(graph: graph)
                Self.mergeBaselineIfPresent(storeURL: url, localGraph: graph, completion: completion)
    ...
    public func needsRun(at phase: GraphMigrationManager.GraphLifecyclePhase, configuration: GraphStoreConfiguration?, graph: Graph?, context: GraphMigrationContext?) -> Bool {
        switch phase {
        case .preInit:
            do {
                // 0. Esiste un file
                ...
                guard fm.fileExists(atPath: config.resolvedStoreURL.path) else { return false }
                
                // 1. Compatibilità Core Data
                let metadata = try NSPersistentStoreCoordinator.metadataForPersistentStore(
                    ofType: NSSQLiteStoreType,
                    at: config.resolvedStoreURL
                )
                let isCompatible = Model.create().isConfiguration(withName: nil, compatibleWithStoreMetadata: metadata)
                
                // 2. Versioni GraphCK / App
                let currentVersions = try GraphStoreMetadata.read(from: config)
                let requiredVersions = config.requiredVersions
    ...
    public func handleRemoteChanges(configuration: GraphStoreConfiguration?, graph: Graph?, inserted: [NSManagedObjectID], updated: [NSManagedObjectID]) {
        ...
        context.perform {
            ...
            let requiredVersion = _graph.configuration.requiredVersions.appData
            var updatedCount = 0
    ...
    internal static func execute(
        ...
        print("📦 Avvio migrazione in-place sullo store: \(configuration.resolvedStoreURL)")
        
        // Backup condizionale
        try preflightBackup(configuration: configuration)
        ...
        try migrateLegacyObjectFieldInPlace(for: configuration)
        ...
        do {
            try GraphStoreMetadata.write(configuration.requiredVersions, to: configuration, model: Model.create())
            
            print("📝 [MigrationV1] Metadata aggiornati: \(configuration.requiredVersions)")
        } catch {
...
internal extension MigrationV1 {
    ...
    static func preflightBackup(configuration: GraphStoreConfiguration) throws {
        let userDefaults = migrationUserDefaults(for: configuration)
        ...
        try backupStoreFiles(at: configuration.storeURL)
        userDefaults.set(true, forKey: backupKey)
        print("[MigrationV1] Backup completato e flag impostato in UserDefaults.")
    }
    
    private static func migrationUserDefaults(for configuration: GraphStoreConfiguration) -> UserDefaults {
        if let suite = UserDefaults(suiteName: configuration.appGroupIdentifier) {
            return suite
        }
        return .standard
    }
...
extension MigrationV1 {
    ...
    static func migrateLegacyObjectField(in context: NSManagedObjectContext, for configuration: GraphStoreConfiguration) throws {
        ...
        for entityName in propertyEntities {
            ...
            Self.migrateProperties(results, in: context, for: configuration)
    ...
    static func migrateLegacyObjectFieldInPlace(for configuration: GraphStoreConfiguration) throws {
        ...
        try context.performAndWait {
            try migrateLegacyObjectField(in: context, for: configuration)
        }
    ...
    private static func migrateProperties(_ objects: [NSManagedObject], in context: NSManagedObjectContext, for configuration: GraphStoreConfiguration) {
        ...
        for object in objects {
            ...
            if object.entity.attributesByName.keys.contains("appDataVersion") {
                let requiredVersion = configuration.requiredVersions.appData
        ...
        // Removed: if context.hasChanges { try context.save() }
            ...
            for entityName in propertyEntities {
                ...
                do {
                    ...
                    for object in results {
                        object.setValue(graph.configuration.requiredVersions.appData, forKey: "appDataVersion")
                    }
                } catch {
